...
W tej sekcji analizujemy, czy dochody różnią się istotnie między regionami przy użyciu testu ANOVA.
...
Jeśli dane nie spełniają założeń normalności, stosujemy test Kruskala-Wallisa.

```{r}
kruskal.test(Income ~ Region, data = dane)
```

## Wykres dla dochodów w różnych regionach

Wizualizacja rozkładu dochodów w różnych regionach.
...
Analiza liczby zakupionych rowerów w różnych regionach.
...
Analiza związku między dochodami a dystansem dojazdu.

```{r}
kruskal.test(Income ~ `Commute Distance`, data = dane)
```

## Wizualizacja dochodów w zależności od dystansu dojazdu

Wykres przedstawiający zależność między dochodami a dystansem dojazdu.
